...
bool TextClass::SetParticleCount(int particles, ID3D11DeviceContext* deviceContext)
{
	char tempString[256];
	char particleString[256];
	bool result;

	// Convert the cpu integer to string format.
	_itoa_s(particles, tempString, 10);

	// Setup the cpu string.
	strcpy_s(particleString, "Current number of particles: ");
	strcat_s(particleString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence11, particleString, 10, 285, 0.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
