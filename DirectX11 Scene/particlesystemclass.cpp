...
bool ParticleSystemClass::InitializeParticleSystem()
{
	...
	m_particleDeviationX = 144.0f; //0.5f;
	m_particleDeviationY = 0.5f; //0.1f;
	m_particleDeviationZ = 144.0f; //2.0f;
...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		positionY = (((float)rand()-(float)rand())/RAND_MAX) * m_particleDeviationY + 21.0f;
		...
		// Make sure that blue is the dominant hue.
		if (blue >= 0.8f) red = blue - 0.1f;
		else if (blue >= 0.5f) red = blue - 0.2f;
		else red = blue/2.0f;
		green = red;
...
void ParticleSystemClass::KillParticles()
{
	...
	for(i=0; i<m_maxParticles; i++)
	{
		if((m_particleList[i].active == true) && (m_particleList[i].positionY < -21.0f))
		{
			m_particleList[i].active = false;
			m_currentParticleCount--;
